...
public class GridLinearLayout extends LinearLayout {
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		TypedArray typedArray = context.obtainStyledAttributes(attrs,
				R.styleable.GridLinearLayout);
		columns = typedArray
				.getInteger(R.styleable.GridLinearLayout_columns, 1);
		rows = typedArray.getInteger(R.styleable.GridLinearLayout_rows, 1);
		horizontalSpace = typedArray.getDimensionPixelSize(
				R.styleable.GridLinearLayout_horizontalSpace, 0);
		verticalSpace = typedArray.getDimensionPixelSize(
				R.styleable.GridLinearLayout_verticalSpace, 0);
		typedArray.recycle();
	}
	...
	private LinearLayout rowLinearLayout() {
		...
		itemLinearLayout.setLayoutParams(new LayoutParams(
	...
	private int screenWidth() {
		/*
		 * Point outSize = new Point(); WindowManager windowManager =
		 * (WindowManager)
		 * getContext().getSystemService(Service.WINDOW_SERVICE); Display
		 * display = windowManager.getDefaultDisplay();
		 * display.getSize(outSize); return outSize.x;
		 */
	...
	/**
	 * 设置当前单元格边距， 每一行第一个左边距为0底边距为horizontalSpace，
